...
namespace 渠道客服系统.Nhome.Ajax.taobao
{
    ...
    /// tbyg 的摘要说明
    /// </summary>
    public class tbyg : IHttpHandler
    {
        ...
        private String[] tbstr(string miliao, string type)
        {
            ...
            cmd.CommandText = string.Format("select * from SQdatatb where 米聊号= '{0}' and 日期 Between '" + start + "' And '" + end + "' order by 日期 asc", name);
        ...
        private string tbTDstr(string time, string type)//团队数据获取
        {
            ...
            Sqlconn.Open();
            SqlCommand cmd = Sqlconn.CreateCommand();
            cmd.CommandText = string.Format("select cast(AVG({0}) as decimal(18,2)) as {1} from SQdatatb where 日期 Between '" + time + "' And '" + time + "' group by 日期 order by 日期 asc", type, type);
            SqlDataAdapter sdr = new SqlDataAdapter(cmd.CommandText, Sqlconn);
            sdr.Fill(dr);
            string str; ;
        ...
        private String[] time(string miliao, string type)
        {
            var date = tbstr(miliao, "日期");
            String[] data = new String[date.Length];
            for (var i = 0; i < date.Length; i++)
            {
                data[i] = tbTDstr(date[i], type);
            }
            return data;
        }
    }
